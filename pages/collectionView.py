            ...
            with st.container(horizontal=True, horizontal_alignment="center", width="stretch"):
                ...
                for i, key in enumerate(items.keys()):
                    ...
                    with col.container(horizontal_alignment="center"):
                        st_yled.subheader(f"{curr_dict['info'].get('name')}", text_alignment="center")

                        if backEnd.CURR_COLL.split("_")[1] == "Pokemon":
                            st.image(items[key]['info']['images']['small'], width=200)
        

                        info = st.text_input("Notes", value = items[key].get('notes'), key = f"notes_{key}")
                        
                        if info != items[key].get('notes'):
                            backEnd.update_notes(db, user_id, key, info)
                            st.success("Updated!")
                            
                        if st_yled.button("View More", key=f"{curr_dict['info'].get('name')}_view"):
                            viewItem(key)
                        st.space("medium")
    else:
        with st.container(horizontal=True, horizontal_alignment="center", width="stretch"):
